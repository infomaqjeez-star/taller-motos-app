...
// Cache en memoria por gerenteId
const cache = new Map<string, { data: any; ts: number }>();
const CACHE_TTL_MS = 120_000; // 2 minutos (dashboard cambia más seguido que productos)
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1) Cache hit
    const cached = cache.get(gerenteId);
    ...
    // 2) Verificar gerente + equipo en paralelo
    const [{ data: gerente }, { data: subordinados }] = await Promise.all([
      supabase.from("vendedores").select("id, nombre, es_gerente").eq("id", gerenteId).single(),
      supabase.from("vendedores").select("id, nombre, email, codigo_referido, nivel_vendedor, comision_pct, estado, lider_id").eq("lider_id", gerenteId),
    ]);
    ...
    if (!equipo.find((v: any) => v.id === gerenteId)) {
      equipo.push(gerente as any);
    }
    ...
    // 3) Pedidos + clientes en paralelo
    const [{ data: pedidos, error: pErr }, { data: clientesEquipo }] = await Promise.all([
      supabase
    ...
    // 5) Guardar cache
    cache.set(gerenteId, { data: result, ts: Date.now() });
