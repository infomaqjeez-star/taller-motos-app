      ...
      <main className="app-main space-y-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex min-w-0 items-start gap-3">
            <Link
              href="/taller"
              className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-gray-300 transition-colors hover:bg-white/10"
              aria-label="Volver al taller"
            >
                ...
                <section key={c.id} id={`cat-${c.id}`} className="scroll-mt-24 space-y-4">
                  ...
                  <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                    {lista.map((p) => (
                      <article
                        key={p.sku}
                        className="card flex flex-col overflow-hidden border border-white/10"
                      >
